...
const formatAIResult = ({rewritten_text}) => rewritten_text;
const calcAIFunction = async () => {
    ...
    const examples = null;
    const response = await client.constrained_text_rewriting(tp, content, content_topic, extra_output_specification, transformation_constraints, examples);
